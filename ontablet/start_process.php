...
try {
    ...
    if ($found_match) {
        ...
        if ($user_data) {
            ...
            // enterstatustbl에 상태 업데이트
            $check_stmt = $conn->prepare("SELECT * FROM enterstatustbl WHERE userid = :userid");
            $check_stmt->bindParam(':userid', $userid);
            $check_stmt->execute();
            
            if ($check_stmt->rowCount() > 0) {
                // 기존 레코드 업데이트
                $update_stmt = $conn->prepare("UPDATE enterstatustbl SET status = 1 WHERE userid = :userid");
                $update_stmt->bindParam(':userid', $userid);
                $update_stmt->execute();
            } else {
                // 새 레코드 삽입
                $insert_stmt = $conn->prepare("INSERT INTO enterstatustbl (userid, status) VALUES (:userid, 1)");
